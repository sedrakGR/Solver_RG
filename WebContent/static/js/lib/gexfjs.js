...
function onGraphClick(evt) {
    if (!GexfJS.mouseHasMoved) {
        ...
        if(GexfJS.params.showSkeleton &&
    	   GexfJS.params.activeNode!=-1 &&
    	   GexfJS.params.activeNode !=  GexfJS.params.lastSkeleton ) {
        	
        	var activeNode = GexfJS.graph.nodeList[GexfJS.params.activeNode].id;
        	var url = 'skeleton.action';
        	$.ajax({
        		url: url,
        		type: 'POST',
        		datatype: 'json',	
        		...
        		success: function(data){
        			 ...
        			 var nodes = data.json.array;
        			 GexfJS.params.skeleton = new Array();
			    	 for( var i = 0;i< nodes.length;i++){
			    		 GexfJS.params.skeleton[i] = 
			    			 GexfJS.graph.nodeIndexById.indexOf(nodes[i]);
			    	 }
        		}
        	});
        	
        	GexfJS.params.lastSkeleton = GexfJS.params.activeNode;
        }
    }
    endMove();
}
...
function loadGraph() {
    $.ajax({
         url: 'manageGraph.action',
         data: {"layoutAlgorithm":layoutAlgorithm},
         type: 'post',
         dataType: 'xml',
        ...
        success: function(data) {
            ...
            $(_edges).each(function() {
                var _e = $(this),
                    ...
                    _col = _e.find("color");
                    _shape = _e.find('attvalue[for="shape"]').attr('value') || _e.attr('shape');;
                    _label = _e.attr("label");
...
function traceArc(contexte, source, target,label) {
    ...
    case "Have":
     ...
     if (GexfJS.params.curvedEdges) {
        if ( ( source.x == target.x ) && ( source.y == target.y ) ) {
            ...
            var middleX = (x3+x4)/2;
            middleX=(middleX+x4)/2;
            var middleY = (y3+y4)/2;
            middleY=(middleY+y4)/2;
            contexte.bezierCurveTo(x3,y3,x4,y4,target.x,target.y);
            contexte.stroke();
            contexte.closePath();
            
            contexte.fillStyle   = contexte.strokeStyle;
            contexte.lineWidth   = 4;
            
            contexte.beginPath();
            var lineangle=Math.atan2(middleY-target.y,middleX-target.x);
            var angle1=lineangle+Math.PI+3.5;
            var angle2=lineangle+Math.PI-3.5;
            var r = target.r*1.3;
            
	         // drowing arrow of the edge.
            contexte.moveTo(target.x, target.y);
            contexte.lineTo(target.x+Math.cos(angle1)*r,target.y+Math.sin(angle1)*r);
            contexte.lineTo(target.x+Math.cos(angle2)*r, target.y+Math.sin(angle2)*r);
...
function traceMap() {
    ...
    GexfJS.params.mousePosition = ( GexfJS.params.useLens ? ( GexfJS.mousePosition ? 
    		( GexfJS.mousePosition.x + "," + GexfJS.mousePosition.y ) : "out" ) : null );
    ...
    for (var i in GexfJS.graph.edgeList) {
        ...
        var _isLinked = false;
        if(!GexfJS.params.showSkeleton || GexfJS.params.skeleton== null || GexfJS.params.skeleton.length == 0 ){
	        ...
	        if (_centralNode != -1) {
	            if (_six == _centralNode) {
	                ...
	                _isLinked = true;
	                //shows the node connected with sorce
	                _dt.visible = true;
	            }
        	...
        	var flag = 0;
        	for(i=0;i< GexfJS.params.skeleton.length; i++ ) {
        		var item = GexfJS.params.skeleton[i];
        		if(item==_six || item ==_tix ) {
        			flag++;
        		} 
        	} if (flag == 2 ) {
        		_ds.visible = true;
        ...
        //checks wether the node or edge are linked to 
        //the central node so to show them or not!
        
        if ( (_isLinked || _displayEdges ) && 
    		 (_ds.withinFrame || _dt.withinFrame ) &&
			  _ds.visible && _dt.visible ) {
            ...
            traceArc(GexfJS.ctxGraphe, _coords, _coordt,_label);
        }
    }
...
function updateButtonStates() {
    ...
    $("#skeletonButton").attr("class",!GexfJS.params.showSkeleton?"":"off")
    .attr("title", strLang(!GexfJS.params.showSkeleton ? "skeletonOff" : "skeletonOn" ) );
}
...
function lunchboxOpen(lunchID) {
	...
	document.getElementById('clasp_' + lunchID).innerHTML="<a href=\"javascript:lunchboxOpen('" + lunchID + "');\">Details...</a>";
	}
	...
	function closeOverlay() {
		 layoutAlgorithm = $('input[name=layoutAlogorithm]:radio:checked').val();
...
$(document).ready(function() {
    ...
    $("#zoomMinusButton").click(function() {
        ...
        $("#zoomSlider").slider("value",GexfJS.params.zoomLevel); //0,54,255 blue
        return false;
    })
    ...
    $("#skeletonButton").click(function () {
    		 GexfJS.params.showSkeleton = !GexfJS.params.showSkeleton;
