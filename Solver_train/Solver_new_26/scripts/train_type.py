#!/usr/bin/env python3
"""Train the piece-type classifier."""
...
from chess_attr.classifiers import TypeClassifier, save_classifier
from chess_attr.dataset import load_square_samples
from chess_attr.evaluation import format_confusion
from chess_attr.training import evaluate_type
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("models/type_model.json"),
        help="Model output path.",
    )
    parser.add_argument(
        "--exclude-none",
        action="store_true",
        help="Train on 6 piece classes only (skip empty squares).",
    )
...
def main() -> None:
    ...
    model = TypeClassifier.train(train_samples, include_none=not args.exclude_none)
    save_classifier(model, args.output)
    print(f"Saved type model -> {args.output}")

    if args.val_csv:
        val_samples = load_square_samples(args.val_csv)
        result = evaluate_type(model, val_samples)
