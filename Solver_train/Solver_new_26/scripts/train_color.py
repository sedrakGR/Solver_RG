#!/usr/bin/env python3
"""Train the color classifier (0:none, 1:side-A, 2:side-B)."""
...
from chess_attr.classifiers import ColorClassifier, save_classifier
from chess_attr.dataset import load_square_samples
from chess_attr.evaluation import format_confusion
from chess_attr.training import evaluate_color
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("models/color_model.json"),
...
def main() -> None:
    ...
    model = ColorClassifier.train(train_samples)
    save_classifier(model, args.output)
    print(f"Saved color model -> {args.output}")

    if args.val_csv:
        val_samples = load_square_samples(args.val_csv)
        result = evaluate_color(model, val_samples)
