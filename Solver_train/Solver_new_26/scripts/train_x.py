#!/usr/bin/env python3
"""Train the X (file index) classifier."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("models/x_model.json"),
        help="Model output path.",
    )
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="Bypass learning and always return crop metadata x value.",
    )
...
def main() -> None:
    ...
    model = AxisClassifier.train(train_samples, axis="x", deterministic=args.deterministic)
    save_classifier(model, args.output)
    print(f"Saved X model -> {args.output}")
